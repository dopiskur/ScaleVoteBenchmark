...
**CPU calibration:** "Run benchmark" measures the node's raw CPU throughput, briefly runs a few real votes with load turned off to see what a vote costs regardless of CPU burn, then suggests a `CpuIterationsPerVote` range that should saturate all cores at a target concurrency `N` you set. CPU saturation is a threshold effect of concurrency vs. core count, not a flat percentage of raw throughput - once concurrent votes exceed the core count, cores queue up regardless of how small `CpuIterationsPerVote` is, so the recommendation solves for the value at which `N` concurrent votes exactly saturate every core. "Set recommended" fills in the Min/Max fields above (still needs "Save changes" to persist); original load settings are restored automatically afterward.
