...
The `Vote` table's primary key (`IDVote` / `id_vote`) is `BIGINT`/`BIGSERIAL` in all three schemas, not a 32-bit `INT`, since this is a load-testing tool expected to accumulate a very large number of rows.
